...
import { Search, CheckCircle, Eye, FileText, FileCode, PlayCircle, Check, AlertTriangle, Camera, Package, Send, Tag, Download } from 'lucide-react';
           ...
           {filteredOrders.map(order => {
               ...
               >
                   ...
                   <div className="pl-3 pt-4 border-t border-gray-100 space-y-3" onClick={(e) => e.stopPropagation()}>
                           ...
                           <div className="mb-2">
                               ...
                               <div className="flex gap-2">
                                    <button 
                                        onClick={(e) => {
                                            e.stopPropagation();
                                            setViewingDoc({ url: order.technicalSheet!, title: `Ficha Técnica #${order.id}` });
                                        }}
                                        className="flex-1 flex items-center justify-center gap-2 px-3 py-2 text-xs font-bold text-white bg-brand-600 hover:bg-brand-700 rounded-lg shadow-sm transition-all hover:shadow-md"
                                    >
                                        <Eye className="w-4 h-4" />
                                        Ver Ficha
                                    </button>
                                    <a 
                                        href={order.technicalSheet} 
                                        download={`ficha-${order.id}.png`}
                                        onClick={(e) => e.stopPropagation()}
                                        className="w-10 flex items-center justify-center bg-gray-100 hover:bg-gray-200 rounded-lg text-gray-600 border border-gray-200 transition-colors"
                                        title="Descargar PDF/Imagen"
                                    >
                       ...
                       {/* MACHINE FILE - Keep as download only usually, but visualizer if needed */}
                       {order.machineFile && (
                           <a href={order.machineFile} download={`bordado-${order.id}.dst`} onClick={(e) => e.stopPropagation()} className="w-full flex items-center justify-center gap-2 px-3 py-2 text-xs font-bold text-gray-700 bg-blue-50 hover:bg-blue-100 border border-blue-200 rounded-lg transition-colors">
                               <FileCode className="w-4 h-4 text-blue-600" /> Descargar Matriz (.DST)
                           </a>
                       )}
                       ...
                       {order.designImage && (
                           <div className="mt-2 pt-2 border-t border-gray-50">
                               <p className="text-[10px] font-bold text-gray-400 uppercase mb-1">Referencia Visual</p>
                               <div 
                                    className="bg-gray-100 rounded-lg p-2 border border-gray-200 cursor-zoom-in group relative overflow-hidden"
                                    onClick={(e) => {
                                        e.stopPropagation();
                                        setViewingDoc({ url: order.designImage!, title: `Diseño Visual #${order.id}` });
                                    }}
                               >
                                   <img 
                                       src={order.designImage} 
                                       className="w-full h-auto max-h-32 object-contain rounded bg-white" 
                                       alt="Referencia de Diseño"
                                   />
                                   <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors flex items-center justify-center">
                                        <Eye className="w-6 h-6 text-white opacity-0 group-hover:opacity-100 drop-shadow-md transform scale-75 group-hover:scale-100 transition-all" />
                                   </div>
                               </div>
                           </div>
                       )}
                       ...
                       <div className="grid grid-cols-1 gap-2 mt-4">
                           {order.status === OrderStatus.READY_TO_EMBROIDER && (
                               <button 
                                   ...
                                   className="w-full py-2 bg-gray-900 hover:bg-black text-white text-xs font-bold rounded-md flex items-center justify-center gap-2 shadow-lg shadow-gray-200"
                               >
                           ...
                           {order.status === OrderStatus.IN_PROGRESS && (
                               <div className="flex gap-2">
                                    <button 
                                        ...
                                        className="flex-1 py-2 bg-orange-500 hover:bg-orange-600 text-white text-xs font-bold rounded-md flex items-center justify-center gap-2 shadow-lg shadow-orange-200"
                                    >
