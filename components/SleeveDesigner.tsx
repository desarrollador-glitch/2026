    ...
    return (
        <div className={`p-4 rounded-xl border ${readOnly ? 'bg-gray-50 border-gray-200' : 'bg-white border-brand-200 shadow-sm'}`}>
            ...
            <div className="space-y-4">
                {/* 1. TEXT INPUT */}
                <div>
                    <label className="text-xs font-bold text-gray-500 mb-1 block">Nombre a Bordar (Opcional)</label>
                    ...
                    <p className="text-[10px] text-gray-400 mt-1">Si el campo está vacío, solo se bordará el icono si se selecciona uno.</p>
                </div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        {SLEEVE_FONTS.map(font => (
                            <button
                                ...
                                className={`p-2 rounded-lg border text-center transition-all ${config.font === font.id
                    ...
                    <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                        {SLEEVE_ICONS.map(icon => (
                            <button
                                ...
                                className={`flex-shrink-0 w-10 h-10 rounded-lg border flex items-center justify-center text-lg transition-all ${config.icon === icon.id
                ...
                <div className="mt-4 pt-4 border-t border-gray-100 text-center">
                    ...
                    <div className="bg-gray-900 text-white p-4 rounded-lg inline-flex items-center gap-2 shadow-inner min-w-[120px] justify-center">
                        <span className="text-xl" style={{
                            ...
                            {config.text || (config.icon === 'NONE' ? 'Personalizar' : '')}
                        </span>
                        {config.icon !== 'NONE' && (
